package gnump3d::HTML::Template;

$gnump3d::HTML::Template::VERSION = '2.8';

=head1 NAME

gnump3d::HTML::Template - Perl module to use HTML Templates from CGI scripts
...
Now create a small CGI program:
  ...
  #!/usr/bin/perl -w
  use gnump3d::HTML::Template;

  # open the html template
  my $template = gnump3d::HTML::Template->new(filename => 'test.tmpl');
...
If you're new to gnump3d::HTML::Template, I suggest you start with the
introductory article available on the gnump3d::HTML::Template website:
...
<TMPL_IF> </TMPL_IF> blocks can include any valid gnump3d::HTML::Template
...
WARNING: Much of the benefit of gnump3d::HTML::Template is in decoupling your
...
gnump3d::HTML::Template's tags are meant to mimic normal HTML tags.  However,
...
gnump3d::HTML::Template processing, or who use DTD-savvy editing tools.
...
Call new() to create a new Template object:
  ...
  my $template = gnump3d::HTML::Template->new( filename => 'file.tmpl', 
                                      option => 'value' 
                                    );
...
use:
  ...
  my $t = gnump3d::HTML::Template->new( scalarref => $ref_to_template_text, 
                               option => 'value' 
                             );

and

  my $t = gnump3d::HTML::Template->new( arrayref => $ref_to_array_of_lines , 
                               option => 'value' 
                             );
...
either traditionally as a glob or as a FileHandle:
  ...
  my $t = gnump3d::HTML::Template->new( filehandle => *FH, option => 'value');

The four new() calling methods can also be accessed as below, if you
prefer.

  my $t = gnump3d::HTML::Template->new_file('file.tmpl', option => 'value');

  my $t = gnump3d::HTML::Template->new_scalar_ref($ref_to_template_text, 
                                        option => 'value');

  my $t = gnump3d::HTML::Template->new_array_ref($ref_to_array_of_lines, 
                                       option => 'value');

  my $t = gnump3d::HTML::Template->new_filehandle($fh, 
                                       option => 'value');

And as a final option, for those that might prefer it, you can call new as:

  my $t = gnump3d::HTML::Template->new(type => 'filename', 
                              source => 'file.tmpl');
...
gnump3d::HTML::Template->new() with filename set to "sam.tmpl", the
gnump3d::HTML::Template will try to open "/home/sam/sam.tmpl" to access the
...
gnump3d::HTML::Template will ignore it.  Defaults to 1.
...
template for all instances of gnump3d::HTML::Template to use.  This can be a
...
By default gnump3d::HTML::Template uses the IPC key 'TMPL' as a shared root
...
gnump3d::HTML::Template see L<IPC::SharedCache>.
...
it have a param() method that works like gnump3d::HTML::Template's param().  A
good candidate would be a CGI.pm query object.  Example:

  my $query = new CGI;
  my $template = gnump3d::HTML::Template->new(filename => 'template.tmpl',
                                     associate => $query);
...
case_sensitive - setting this option to true causes gnump3d::HTML::Template to
...
gnump3d::HTML::Template reads your template file but before it starts parsing
template tags.
...
FOO!!!" and transforms them into gnump3d::HTML::Template tags:
   ...
   my $template = gnump3d::HTML::Template->new(filename => 'zap.tmpl',
                                      filter => $filter);
...
gnump3d::HTML::Template will apply the specified escaping to all variables
...
package gnump3d::HTML::Template::LOOP;
sub TEMPLATE_HASH () { 0 };
sub PARAM_SET     () { 1 };

package gnump3d::HTML::Template::COND;
sub VARIABLE           () { 0 };
sub VARIABLE_TYPE      () { 1 };
...
sub new {
  ...
  for (my $x = 0; $x <= $#_; $x += 2) {
    defined($_[($x + 1)]) or croak("gnump3d::HTML::Template->new() called with odd number of option parameters - should be of the form option => value");
  ...
  if (exists($options->{type})) {
    exists($options->{source}) or croak("gnump3d::HTML::Template->new() called with 'type' parameter set, but no 'source'!");
    ($options->{type} eq 'filename' or $options->{type} eq 'scalarref' or
     $options->{type} eq 'arrayref' or $options->{type} eq 'filehandle') or
       croak("gnump3d::HTML::Template->new() : type parameter must be set to 'filename', 'arrayref', 'scalarref' or 'filehandle'!");
  ...
  foreach my $object (@{$options->{associate}}) {
    defined($object->can('param')) or
      croak("gnump3d::HTML::Template->new called with associate option, containing object of type " . ref($object) . " which lacks a param() method!");
  } 
  ...
  if ($source_count != 1) {
    croak("gnump3d::HTML::Template->new called with multiple (or no) template sources specified!  A valid call to new() has exactly one filename => 'file' OR exactly one scalarref => \\\$scalar OR exactly one arrayref => \\\@array OR exactly one filehandle => \*FH");
  }

  # check that filenames aren't empty
  if (exists($options->{filename})) {
      croak("gnump3d::HTML::Template->new called with empty filename parameter!")
  ...
  if ($options->{memory_debug}) {
    ...
    eval { require GTop; };
    croak("Could not load GTop.  You must have GTop installed to use gnump3d::HTML::Template in memory_debug mode.  The error was: $@")
      if ($@);
    $self->{gtop} = GTop->new();
    $self->{proc_mem} = $self->{gtop}->proc_mem($$);
    print STDERR "\n### gnump3d::HTML::Template Memory Debug ### START ", $self->{proc_mem}->size(), "\n";
  }
  ...
  if ($options->{file_cache}) {
    ...
    croak("Could not load Storable.  You must have Storable installed to use gnump3d::HTML::Template in file_cache mode.  The error was: $@")
      if ($@);
  }
  ...
  if ($options->{shared_cache}) {
    ...
    croak("Could not load IPC::SharedCache.  You must have IPC::SharedCache installed to use gnump3d::HTML::Template in shared_cache mode.  The error was: $@")
      if ($@);
  ...
  if ($options->{default_escape}) {
    ...
    unless ($options->{default_escape} =~ /^(HTML|URL|JS)$/) {
      croak("gnump3d::HTML::Template->new(): Invalid setting for default_escape - '$options->{default_escape}'.  Valid values are HTML, URL or JS.");
    }
  }

  print STDERR "### gnump3d::HTML::Template Memory Debug ### POST CACHE INIT ", $self->{proc_mem}->size(), "\n"
    if $options->{memory_debug};

  # initialize data structures
  $self->_init;
  
  print STDERR "### gnump3d::HTML::Template Memory Debug ### POST _INIT CALL ", $self->{proc_mem}->size(), "\n"
    if $options->{memory_debug};
...
sub _fetch_from_cache {
    ...
    if (exists($CACHE{$key}{included_mtimes})) {
      foreach my $filename (keys %{$CACHE{$key}{included_mtimes}}) {
        ...
        if ($included_mtime != $CACHE{$key}{included_mtimes}{$filename}) {
          $options->{cache_debug} and 
            print STDERR "### gnump3d::HTML::Template Cache Debug ### CACHE MISS : $filepath : INCLUDE $filename : $included_mtime\n";
  ...
  $options->{cache_debug} and print STDERR "### gnump3d::HTML::Template Cache Debug ### CACHE HIT : $filepath => $key\n";
...
sub _commit_to_cache {
  ...
  $options->{cache_debug} and print STDERR "### gnump3d::HTML::Template Cache Debug ### CACHE LOAD : $filepath => $key\n";
...
sub _cache_key {
    ...
    if (not defined $filepath) {
        ...
        confess("gnump3d::HTML::Template->new() : Cannot find file '$options->{filename}'.")
...
sub _fetch_from_file_cache {
  ...
  croak("gnump3d::HTML::Template::new() - Problem reading cache file $cache_filename (file_cache => 1) : $@")
    if $@;
  croak("gnump3d::HTML::Template::new() - Problem reading cache file $cache_filename (file_cache => 1) : $!") 
  ...
  if (defined $mtime) {
    ...
    if (defined($self->{mtime}) and 
        ($mtime != $self->{mtime})) {
      $options->{cache_debug} and 
        print STDERR "### gnump3d::HTML::Template Cache Debug ### FILE CACHE MISS : $filepath : $mtime\n";
      ($self->{mtime}, 
       $self->{included_mtimes}, 
       $self->{param_map}, 
    ...
    if (exists($self->{included_mtimes})) {
      foreach my $filename (keys %{$self->{included_mtimes}}) {
        ...
        if ($included_mtime != $self->{included_mtimes}{$filename}) {
          $options->{cache_debug} and 
            print STDERR "### gnump3d::HTML::Template Cache Debug ### FILE CACHE MISS : $filepath : INCLUDE $filename : $included_mtime\n";
          ($self->{mtime}, 
           $self->{included_mtimes}, 
           $self->{param_map}, 
           $self->{parse_stack}) = (undef, undef, undef, undef);
  ...
  # got a cache hit!
  $options->{cache_debug} and print STDERR "### gnump3d::HTML::Template Cache Debug ### FILE CACHE HIT : $filepath\n";
...
sub _commit_to_file_cache {
  ...
  if (not defined $filepath) {
    $filepath = $self->_find_file($options->{filename});
    confess("gnump3d::HTML::Template->new() : Cannot open included file $options->{filename} : file not found.")
      unless defined($filepath);
    $options->{filepath} = $filepath;   
  }
  ...
  if (not -d $cache_dir) {
	...
	or croak("gnump3d::HTML::Template->new() : can't mkdir $options->{file_cache_dir} (file_cache => 1): $!");
    }
    mkdir($cache_dir,$options->{file_cache_dir_mode})
      or croak("gnump3d::HTML::Template->new() : can't mkdir $cache_dir (file_cache => 1): $!");
  }

  $options->{cache_debug} and print STDERR "### gnump3d::HTML::Template Cache Debug ### FILE CACHE LOAD : $options->{filepath}\n";
  ...
  croak("gnump3d::HTML::Template::new() - Problem writing cache file $cache_dir/$cache_file (file_cache => 1) : $@")
    if $@;
  croak("gnump3d::HTML::Template::new() - Problem writing cache file $cache_dir/$cache_file (file_cache => 1) : $!")
...
sub _fetch_from_shared_cache {
  ...
  $options->{cache_debug} and defined($self->{record}) and print STDERR "### gnump3d::HTML::Template Cache Debug ### CACHE HIT : $filepath\n";
  # clear out values from param_map from last run
...
sub _validate_shared_cache {
  ...
  $options->{shared_cache_debug} and print STDERR "### gnump3d::HTML::Template Cache Debug ### SHARED CACHE VALIDATE : $filename\n";
  ...
  if (defined $mtime and defined $c_mtime
      and $mtime != $c_mtime) {
    $options->{cache_debug} and 
      print STDERR "### gnump3d::HTML::Template Cache Debug ### SHARED CACHE MISS : $filename : $mtime\n";
    return 0;
  }
  ...
  if (defined $mtime and defined $included_mtimes) {
    foreach my $fname (keys %$included_mtimes) {
      ...
      if ($included_mtimes->{$fname} != (stat($fname))[9]) {
        $options->{cache_debug} and 
          print STDERR "### gnump3d::HTML::Template Cache Debug ### SHARED CACHE MISS : $filename : INCLUDE $fname\n";
        return 0;
      }
    }
  }
...
sub _load_shared_cache {
  ...
  $options->{cache_debug} and print STDERR "### gnump3d::HTML::Template Cache Debug ### SHARED CACHE LOAD : $options->{filepath}\n";
  
  print STDERR "### gnump3d::HTML::Template Memory Debug ### END CACHE LOAD ", $self->{proc_mem}->size(), "\n"
    if $options->{memory_debug};
...
sub _mtime {
  ...
  (-r $filepath) or Carp::confess("gnump3d::HTML::Template : template file $filepath does not exist or is unreadable.");
...
sub _normalize_options {
  ...
  while(@pstacks) {
    ...
    foreach my $item (@$pstack) {
      next unless (ref($item) eq 'gnump3d::HTML::Template::LOOP');
      foreach my $template (values %{$item->[gnump3d::HTML::Template::LOOP::TEMPLATE_HASH]}) {
...
sub _init_template {
  ...
  print STDERR "### gnump3d::HTML::Template Memory Debug ### START INIT_TEMPLATE ", $self->{proc_mem}->size(), "\n"
    if $options->{memory_debug};
  ...
  if (exists($options->{filename})) {    
    ...
    confess("gnump3d::HTML::Template->new() : Cannot open included file $options->{filename} : $!")
    ...
    confess("gnump3d::HTML::Template : Need to call new with filename, filehandle, scalarref or arrayref parameter specified.");
  }

  print STDERR "### gnump3d::HTML::Template Memory Debug ### END INIT_TEMPLATE ", $self->{proc_mem}->size(), "\n"
    if $options->{memory_debug};
...
sub _call_filters {
  ...
  foreach my $filter (@{$options->{filter}}) {
    croak("gnump3d::HTML::Template->new() : bad value set for filter parameter - must be a code ref or a hash ref.")
      unless ref $filter;
    ...
    if (ref $filter eq 'HASH') {
      ...
      croak("gnump3d::HTML::Template->new() : bad value set for filter parameter - hash must contain \"format\" key and \"sub\" key.")
        unless defined $format and defined $sub;
      croak("gnump3d::HTML::Template->new() : bad value set for filter parameter - \"format\" must be either 'array' or 'scalar'")
        unless $format eq 'array' or $format eq 'scalar';
      croak("gnump3d::HTML::Template->new() : bad value set for filter parameter - \"sub\" must be a code ref")
      ...
      croak("gnump3d::HTML::Template->new() : fatal error occured during filter call: $@") if $@;
    } else {
      croak("gnump3d::HTML::Template->new() : bad value set for filter parameter - must be code ref or hash ref");
    }
  }
  # all done
  return $template_ref;
...
sub _parse {
  ...
  $options->{debug} and print STDERR "### gnump3d::HTML::Template Debug ### In _parse:\n";
  ...
  *ucstack = $ucstacks[0];
  ...
  my $NOOP = gnump3d::HTML::Template::NOOP->new();
  my $ESCAPE = gnump3d::HTML::Template::ESCAPE->new();
  my $JSESCAPE = gnump3d::HTML::Template::JSESCAPE->new();
  my $URLESCAPE = gnump3d::HTML::Template::URLESCAPE->new();
 ...
 CHUNK: for (my $chunk_number = 0;
    ...
    if ($chunk =~ /^<
      ...
      die "gnump3d::HTML::Template->new() : No NAME given to a $which tag at $fname : line $fcounter." 
        if ($need_names{$which} and (not defined $name or not length $name));

      # die if we got an escape but can't use one
      die "gnump3d::HTML::Template->new() : ESCAPE option invalid in a $which tag at $fname : line $fcounter." if ( $escape and ($which ne 'TMPL_VAR'));

      # die if we got a default but can't use one
      die "gnump3d::HTML::Template->new() : DEFAULT option invalid in a $which tag at $fname : line $fcounter." if ( defined $default and ($which ne 'TMPL_VAR'));
        
      # take actions depending on which tag found
      if ($which eq 'TMPL_VAR') {
	$options->{debug} and print STDERR "### gnump3d::HTML::Template Debug ### $fname : line $fcounter : parsed VAR $name\n";
	
	# if we already have this var, then simply link to the existing
	# gnump3d::HTML::Template::VAR, else create a new one.        
	my $var;        
	if (exists $pmap{$name}) {
	  $var = $pmap{$name};
	  (ref($var) eq 'gnump3d::HTML::Template::VAR') or
	    die "gnump3d::HTML::Template->new() : Already used param name $name as a TMPL_LOOP, found in a TMPL_VAR at $fname : line $fcounter.";
	} else {
	  $var = gnump3d::HTML::Template::VAR->new();
	  $pmap{$name} = $var;
	  $top_pmap{$name} = gnump3d::HTML::Template::VAR->new()
	...
	if (defined $default) {
            push(@pstack, gnump3d::HTML::Template::DEFAULT->new($default));
        }
	...
	$options->{debug} and print STDERR "### gnump3d::HTML::Template Debug ### $fname : line $fcounter : LOOP $name start\n";
	
	# if we already have this loop, then simply link to the existing
	# gnump3d::HTML::Template::LOOP, else create a new one.
	my $loop;
	if (exists $pmap{$name}) {
	  $loop = $pmap{$name};
	  (ref($loop) eq 'gnump3d::HTML::Template::LOOP') or
	    die "gnump3d::HTML::Template->new() : Already used param name $name as a TMPL_VAR, TMPL_IF or TMPL_UNLESS, found in a TMP_LOOP at $fname : line $fcounter!";
	  
	} else {
	  # store the results in a LOOP object - actually just a
	  # thin wrapper around another gnump3d::HTML::Template object.
	  $loop = gnump3d::HTML::Template::LOOP->new();
	  $pmap{$name} = $loop;
	}
	...
	*ucstack = $ucstacks[$#ucstacks];
	...
	if ($options->{loop_context_vars}) {
	  $pmap{__first__}   = gnump3d::HTML::Template::VAR->new();
	  $pmap{__inner__}   = gnump3d::HTML::Template::VAR->new();
	  $pmap{__last__}    = gnump3d::HTML::Template::VAR->new();
	  $pmap{__odd__}     = gnump3d::HTML::Template::VAR->new();
	  $pmap{__counter__} = gnump3d::HTML::Template::VAR->new();
	}
	
      } elsif ($which eq '/TMPL_LOOP') {
	$options->{debug} and print STDERR "### gnump3d::HTML::Template Debug ### $fname : line $fcounter : LOOP end\n";
	
	my $loopdata = pop(@loopstack);
	die "gnump3d::HTML::Template->new() : found </TMPL_LOOP> with no matching <TMPL_LOOP> at $fname : line $fcounter!" unless defined $loopdata;
	
	my ($loop, $starts_at) = @$loopdata;
	
	# resolve pending conditionals
	foreach my $uc (@ucstack) {
	  my $var = $uc->[gnump3d::HTML::Template::COND::VARIABLE]; 
	  if (exists($pmap{$var})) {
	    $uc->[gnump3d::HTML::Template::COND::VARIABLE] = $pmap{$var};
	  } else {
	    $pmap{$var} = gnump3d::HTML::Template::VAR->new();
	    $top_pmap{$var} = gnump3d::HTML::Template::VAR->new()
	      if $options->{global_vars} and not exists $top_pmap{$var};
	    $uc->[gnump3d::HTML::Template::COND::VARIABLE] = $pmap{$var};
	  }
	  if (ref($pmap{$var}) eq 'gnump3d::HTML::Template::VAR') {
	    $uc->[gnump3d::HTML::Template::COND::VARIABLE_TYPE] = gnump3d::HTML::Template::COND::VARIABLE_TYPE_VAR;
	  } else {
	    $uc->[gnump3d::HTML::Template::COND::VARIABLE_TYPE] = gnump3d::HTML::Template::COND::VARIABLE_TYPE_LOOP;
	  }
	}
	...
	*pstack = $pstacks[$#pstacks];
	...
	scalar(@ifstack) and die "gnump3d::HTML::Template->new() : Dangling <TMPL_IF> or <TMPL_UNLESS> in loop ending at $fname : line $fcounter.";
	pop(@ifstacks);
	*ifstack = $ifstacks[$#ifstacks];
	pop(@ucstacks);
	*ucstack = $ucstacks[$#ucstacks];
	...
	$loop->[gnump3d::HTML::Template::LOOP::TEMPLATE_HASH]{$starts_at}             
	...
	$options->{debug} and print STDERR "### gnump3d::HTML::Template Debug ### $fname : line $fcounter : $which $name start\n";
	
	# if we already have this var, then simply link to the existing
	# gnump3d::HTML::Template::VAR/LOOP, else defer the mapping
	my $var;        
	if (exists $pmap{$name}) {
	  $var = $pmap{$name};
	} else {
	...
	my $cond = gnump3d::HTML::Template::COND->new($var);
	if ($which eq 'TMPL_IF') {
	  $cond->[gnump3d::HTML::Template::COND::WHICH] = gnump3d::HTML::Template::COND::WHICH_IF;
	  $cond->[gnump3d::HTML::Template::COND::JUMP_IF_TRUE] = 0;
	} else {
	  $cond->[gnump3d::HTML::Template::COND::WHICH] = gnump3d::HTML::Template::COND::WHICH_UNLESS;
	  $cond->[gnump3d::HTML::Template::COND::JUMP_IF_TRUE] = 1;
	}
	...
	if ($var eq $name) {
	  ...
	  if (ref($var) eq 'gnump3d::HTML::Template::VAR') {
	    $cond->[gnump3d::HTML::Template::COND::VARIABLE_TYPE] = gnump3d::HTML::Template::COND::VARIABLE_TYPE_VAR;
	  } else {
	    $cond->[gnump3d::HTML::Template::COND::VARIABLE_TYPE] = gnump3d::HTML::Template::COND::VARIABLE_TYPE_LOOP;
	  }
	}
	...
	$options->{debug} and print STDERR "### gnump3d::HTML::Template Debug ### $fname : line $fcounter : $which end\n";
	
	my $cond = pop(@ifstack);
	die "gnump3d::HTML::Template->new() : found </${which}> with no matching <TMPL_IF> at $fname : line $fcounter." unless defined $cond;
	if ($which eq '/TMPL_IF') {
	  die "gnump3d::HTML::Template->new() : found </TMPL_IF> incorrectly terminating a <TMPL_UNLESS> (use </TMPL_UNLESS>) at $fname : line $fcounter.\n" 
	    if ($cond->[gnump3d::HTML::Template::COND::WHICH] == gnump3d::HTML::Template::COND::WHICH_UNLESS);
	} else {
	  die "gnump3d::HTML::Template->new() : found </TMPL_UNLESS> incorrectly terminating a <TMPL_IF> (use </TMPL_IF>) at $fname : line $fcounter.\n" 
	    if ($cond->[gnump3d::HTML::Template::COND::WHICH] == gnump3d::HTML::Template::COND::WHICH_IF);
	}
	...
	push(@pstack, $NOOP);
	$cond->[gnump3d::HTML::Template::COND::JUMP_ADDRESS] = $#pstack;
	
      } elsif ($which eq 'TMPL_ELSE') {
	$options->{debug} and print STDERR "### gnump3d::HTML::Template Debug ### $fname : line $fcounter : ELSE\n";
	
	my $cond = pop(@ifstack);
	die "gnump3d::HTML::Template->new() : found <TMPL_ELSE> with no matching <TMPL_IF> or <TMPL_UNLESS> at $fname : line $fcounter." unless defined $cond;
        die "gnump3d::HTML::Template->new() : found second <TMPL_ELSE> tag for  <TMPL_IF> or <TMPL_UNLESS> at $fname : line $fcounter." if $cond->[gnump3d::HTML::Template::COND::IS_ELSE];	
	
	my $else = gnump3d::HTML::Template::COND->new($cond->[gnump3d::HTML::Template::COND::VARIABLE]);
	$else->[gnump3d::HTML::Template::COND::WHICH] = $cond->[gnump3d::HTML::Template::COND::WHICH];
        $else->[gnump3d::HTML::Template::COND::UNCONDITIONAL_JUMP] = 1;
	$else->[gnump3d::HTML::Template::COND::IS_ELSE] = 1;

	# need end-block resolution?
	if (defined($cond->[gnump3d::HTML::Template::COND::VARIABLE_TYPE])) {
	  $else->[gnump3d::HTML::Template::COND::VARIABLE_TYPE] = $cond->[gnump3d::HTML::Template::COND::VARIABLE_TYPE];
	} else {
	  push(@ucstack, $else);
	}
	...
	$options->{debug} and print STDERR "### gnump3d::HTML::Template Debug ### $fname : line $fcounter : INCLUDE $name \n";
	
	# no includes here, bub
	$options->{no_includes} and croak("gnump3d::HTML::Template : Illegal attempt to use TMPL_INCLUDE in template file : (no_includes => 1)");
	...
	die "gnump3d::HTML::Template->new() : Cannot open included file $filename : file not found."
	  unless defined($filepath);
	die "gnump3d::HTML::Template->new() : Cannot open included file $filename : $!"
	  unless defined(open(TEMPLATE, $filepath));              
	...
	if ($included_template) { # not empty
	  ...
	  (*fname, *fcounter, *fmax) = \ ( @{$fstack[$#fstack]} );
          ...
          die "gnump3d::HTML::Template->new() : likely recursive includes - parsed $options->{max_includes} files deep and giving up (set max_includes higher to allow deeper recursion)." if ($options->{max_includes} and (scalar(@fstack) > $options->{max_includes}));
	...
	# zuh!?
	die "gnump3d::HTML::Template->new() : Unknown or unmatched TMPL construct at $fname : line $fcounter.";
      }
	...
	if (ref($pstack[$#pstack]) eq 'SCALAR') {
      ...
      if ($options->{strict}) {
	die "gnump3d::HTML::Template->new() : Syntax error in <TMPL_*> tag at $fname : $fcounter." if ($chunk =~ /<(?:!--\s*)?\/?[Tt][Mm][Pp][Ll]_/);
      }
  ...
  scalar(@ifstack) and die "gnump3d::HTML::Template->new() : At least one <TMPL_IF> or <TMPL_UNLESS> not terminated at end of file!";
  scalar(@loopstack) and die "gnump3d::HTML::Template->new() : At least one <TMPL_LOOP> not terminated at end of file!";
  ...
  if ($options->{stack_debug}) {
    ...
    print STDERR "### gnump3d::HTML::Template _param Stack Dump ###\n\n", Data::Dumper::Dumper($self->{parse_stack}), "\n";
  }
...
sub _globalize_vars {
  ...
  map { $_->_globalize_vars(@_) } 
    map {values %{$_->[gnump3d::HTML::Template::LOOP::TEMPLATE_HASH]}}
      grep { ref($_) eq 'gnump3d::HTML::Template::LOOP'} @{$self->{parse_stack}};
}
...
sub param {
  ...
  if (!scalar(@_) and !length($type)) {
    ...
    $options->{die_on_bad_params} and !exists($param_map->{$param}) and
      croak("gnump3d::HTML::Template : Attempt to get nonexistent parameter '$param' - this parameter name doesn't match any declarations in the template file : (die_on_bad_params set => 1)");
    
    return undef unless (exists($param_map->{$param}) and
                         defined($param_map->{$param}));

    return ${$param_map->{$param}} if 
      (ref($param_map->{$param}) eq 'gnump3d::HTML::Template::VAR');
    return $param_map->{$param}[gnump3d::HTML::Template::LOOP::PARAM_SET];
  } 

  if (!scalar(@_)) {
    croak("gnump3d::HTML::Template->param() : Single reference arg to param() must be a hash-ref!  You gave me a $type.")
  ...
  croak("gnump3d::HTML::Template->param() : You gave me an odd number of parameters to param()!")
    unless ((@_ % 2) == 0);
  ...
  for (my $x = 0; $x <= $#_; $x += 2) {
    ...
    $options->{die_on_bad_params} and !exists($param_map->{$param}) and
      croak("gnump3d::HTML::Template : Attempt to set nonexistent parameter '$param' - this parameter name doesn't match any declarations in the template file : (die_on_bad_params => 1)");
    ...
    if (defined($value_type) and length($value_type) and ($value_type eq 'ARRAY' or ((ref($value) !~ /^(CODE)|(HASH)|(SCALAR)$/) and $value->isa('ARRAY')))) {
      (ref($param_map->{$param}) eq 'gnump3d::HTML::Template::LOOP') or
        croak("gnump3d::HTML::Template::param() : attempt to set parameter '$param' with an array ref - parameter is not a TMPL_LOOP!");
      $param_map->{$param}[gnump3d::HTML::Template::LOOP::PARAM_SET] = [@{$value}];
    } else {
      (ref($param_map->{$param}) eq 'gnump3d::HTML::Template::VAR') or
        croak("gnump3d::HTML::Template::param() : attempt to set parameter '$param' with a scalar - parameter is not a TMPL_VAR!");
...
sub clear_params {
  ...
  foreach my $name (keys %{$self->{param_map}}) {
    ...
    undef(${$self->{param_map}{$name}})
      if ($type eq 'gnump3d::HTML::Template::VAR');
    undef($self->{param_map}{$name}[gnump3d::HTML::Template::LOOP::PARAM_SET])
      if ($type eq 'gnump3d::HTML::Template::LOOP');    
  }
}
...
sub associateCGI { 
  ...
  my $cgi  = shift;
  (ref($cgi) eq 'CGI') or
    croak("Warning! non-CGI object was passed to gnump3d::HTML::Template::associateCGI()!\n");
...
sub output {
  ...
  croak("gnump3d::HTML::Template->output() : You gave me an odd number of parameters to output()!")
    unless ((@_ % 2) == 0);
  my %args = @_;

  print STDERR "### gnump3d::HTML::Template Memory Debug ### START OUTPUT ", $self->{proc_mem}->size(), "\n"
    if $options->{memory_debug};

  $options->{debug} and print STDERR "### gnump3d::HTML::Template Debug ### In output\n";

  # want a stack dump?
  if ($options->{stack_debug}) {
    require 'Data/Dumper.pm';
    print STDERR "### gnump3d::HTML::Template output Stack Dump ###\n\n", Data::Dumper::Dumper($self->{parse_stack}), "\n";
  }
  ...
  *parse_stack = $self->{parse_stack};
  ...
  tie $result, 'gnump3d::HTML::Template::PRINTSCALAR', $args{print_to}
  ...
  for (my $x = 0; $x <= $parse_stack_length; $x++) {
    *line = \$parse_stack[$x];
    ...
    if ($type eq 'SCALAR') {
      $result .= $$line;
    } elsif ($type eq 'gnump3d::HTML::Template::VAR' and ref($$line) eq 'CODE') {
      defined($$line) and $result .= $$line->($self);
    } elsif ($type eq 'gnump3d::HTML::Template::VAR') {
      defined($$line) and $result .= $$line;
    } elsif ($type eq 'gnump3d::HTML::Template::LOOP') {
      if (defined($line->[gnump3d::HTML::Template::LOOP::PARAM_SET])) {
        eval { $result .= $line->output($x, $options->{loop_context_vars}); };
        croak("gnump3d::HTML::Template->output() : fatal error in loop output : $@") 
          if $@;
      }
	} elsif ($type eq 'gnump3d::HTML::Template::COND') {
    	
     if ($line->[gnump3d::HTML::Template::COND::UNCONDITIONAL_JUMP]) {
       $x = $line->[gnump3d::HTML::Template::COND::JUMP_ADDRESS]
     } else {
        if ($line->[gnump3d::HTML::Template::COND::JUMP_IF_TRUE]) {
          if ($line->[gnump3d::HTML::Template::COND::VARIABLE_TYPE] == gnump3d::HTML::Template::COND::VARIABLE_TYPE_VAR) {
            if (defined ${$line->[gnump3d::HTML::Template::COND::VARIABLE]}) {
              if (ref(${$line->[gnump3d::HTML::Template::COND::VARIABLE]}) eq 'CODE') {
                $x = $line->[gnump3d::HTML::Template::COND::JUMP_ADDRESS] if ${$line->[gnump3d::HTML::Template::COND::VARIABLE]}->($self);
              } else {
                $x = $line->[gnump3d::HTML::Template::COND::JUMP_ADDRESS] if ${$line->[gnump3d::HTML::Template::COND::VARIABLE]};
              }
            }
          } else {
            $x = $line->[gnump3d::HTML::Template::COND::JUMP_ADDRESS] if
              (defined $line->[gnump3d::HTML::Template::COND::VARIABLE][gnump3d::HTML::Template::LOOP::PARAM_SET] and
               scalar @{$line->[gnump3d::HTML::Template::COND::VARIABLE][gnump3d::HTML::Template::LOOP::PARAM_SET]});
          }
        } else {
          if ($line->[gnump3d::HTML::Template::COND::VARIABLE_TYPE] == gnump3d::HTML::Template::COND::VARIABLE_TYPE_VAR) {
            if (defined ${$line->[gnump3d::HTML::Template::COND::VARIABLE]}) {
              if (ref(${$line->[gnump3d::HTML::Template::COND::VARIABLE]}) eq 'CODE') {
                $x = $line->[gnump3d::HTML::Template::COND::JUMP_ADDRESS] unless ${$line->[gnump3d::HTML::Template::COND::VARIABLE]}->($self);
              } else {
                $x = $line->[gnump3d::HTML::Template::COND::JUMP_ADDRESS] unless ${$line->[gnump3d::HTML::Template::COND::VARIABLE]};
              }
            } else {
              $x = $line->[gnump3d::HTML::Template::COND::JUMP_ADDRESS];
            }
            ...
            $x = $line->[gnump3d::HTML::Template::COND::JUMP_ADDRESS] if
              (not defined $line->[gnump3d::HTML::Template::COND::VARIABLE][gnump3d::HTML::Template::LOOP::PARAM_SET] or
               not scalar @{$line->[gnump3d::HTML::Template::COND::VARIABLE][gnump3d::HTML::Template::LOOP::PARAM_SET]});
          }
        }
      }      	
    } elsif ($type eq 'gnump3d::HTML::Template::NOOP') {
      next;
    } elsif ($type eq 'gnump3d::HTML::Template::DEFAULT') {
      $_ = $x;  # remember default place in stack

      # find next VAR, there might be an ESCAPE in the way
      *line = \$parse_stack[++$x];
      *line = \$parse_stack[++$x] if ref $line eq 'gnump3d::HTML::Template::ESCAPE';
      ...
      next;      
    } elsif ($type eq 'gnump3d::HTML::Template::ESCAPE') {
      *line = \$parse_stack[++$x];
      if (defined($$line)) {
        if (ref($$line) eq 'CODE') {
            $_ = $$line->($self);
      ...
      next;
    } elsif ($type eq 'gnump3d::HTML::Template::JSESCAPE') {
      $x++;
      *line = \$parse_stack[$x];
      if (defined($$line)) {
        if (ref($$line) eq 'CODE') {
    ...
    } elsif ($type eq 'gnump3d::HTML::Template::URLESCAPE') {
      $x++;
      *line = \$parse_stack[$x];
      if (defined($$line)) {
        if (ref($$line) eq 'CODE') {
      ...
      confess("gnump3d::HTML::Template::output() : Unknown item in parse_stack : " . $type);
    }
  }
  ...
  print STDERR "### gnump3d::HTML::Template Memory Debug ### END OUTPUT ", $self->{proc_mem}->size(), "\n"
    if $options->{memory_debug};
...
sub query {
  my $self = shift;
  $self->{options}{debug} and print STDERR "### gnump3d::HTML::Template Debug ### query(", join(', ', @_), ")\n";

  # the no-parameter case - return $self->param()
  return $self->param() unless scalar(@_);
  
  croak("gnump3d::HTML::Template::query() : Odd number of parameters passed to query!")
    if (scalar(@_) % 2);
  croak("gnump3d::HTML::Template::query() : Wrong number of parameters passed to query - should be 2.")
    if (scalar(@_) != 2);

  my ($opt, $path) = (lc shift, shift);
  croak("gnump3d::HTML::Template::query() : invalid parameter ($opt)")
  ...
  if ($opt eq 'name') {
    ...
    return 'VAR' if $type eq 'gnump3d::HTML::Template::VAR';
    return 'LOOP' if $type eq 'gnump3d::HTML::Template::LOOP';
    croak("gnump3d::HTML::Template::query() : unknown object ($type) in param_map!");

  } elsif ($opt eq 'loop') {
    my %results;
    while(@objs) {
      ($obj, $type) = (shift(@objs), shift(@objs));
      croak("gnump3d::HTML::Template::query() : Search path [", join(', ', @$path), "] doesn't end in a TMPL_LOOP - it is an error to use the 'loop' option on a non-loop parameter.  To avoid this problem you can use the 'name' option to query() to check the type first.") 
        unless ((defined $obj) and ($type eq 'gnump3d::HTML::Template::LOOP'));
      ...
      map {$results{$_} = 1} map { keys(%{$_->{'param_map'}}) }
        values(%{$obj->[gnump3d::HTML::Template::LOOP::TEMPLATE_HASH]});
    }
...
sub _find_param {
  ...
  return unless ($type eq 'gnump3d::HTML::Template::LOOP');
...
# gnump3d::HTML::Template::VAR, LOOP, etc are *light* objects - their internal
# spec is used above.  No encapsulation or information hiding is to be
# assumed.

package gnump3d::HTML::Template::VAR;
...
package gnump3d::HTML::Template::DEFAULT;
...
package gnump3d::HTML::Template::NOOP;
sub new {
  my $unused;
  my $self = \$unused;
  bless($self, $_[0]);
...
package gnump3d::HTML::Template::ESCAPE;
sub new {
  my $unused;
  my $self = \$unused;
  bless($self, $_[0]);
...
package gnump3d::HTML::Template::JSESCAPE;
sub new {
  my $unused;
  my $self = \$unused;
  bless($self, $_[0]);
...
package gnump3d::HTML::Template::URLESCAPE;
sub new {
  my $unused;
  my $self = \$unused;
  bless($self, $_[0]);
...
package gnump3d::HTML::Template::PRINTSCALAR;
use strict;
...
Q: Is there a place to go to discuss gnump3d::HTML::Template and/or get help?

A: There's a mailing-list for discussing gnump3d::HTML::Template at
...
gnump3d::HTML::Template on the mailing list.  Please be ready to explain to me
how the new tag fits in with gnump3d::HTML::Template's mission to provide a
fast, lightweight system for using HTML templates.

NOTE: Offering to program said addition and provide it in the form of
a patch to the most recent version of gnump3d::HTML::Template will definitely
...
A: That depends.  Did you send me the VERSION of gnump3d::HTML::Template, a test
script and a test template?  If so, then almost certainly.

If you're feeling really adventurous, gnump3d::HTML::Template has a publically
...
same path you use when you call gnump3d::HTML::Template->new().  Otherwise the
...
violates the fundamental concept behind gnump3d::HTML::Template - that design
...
L<gnump3d::HTML::Template::Expr|gnump3d::HTML::Template::Expr>.  Using
gnump3d::HTML::Template::Expr it should be easy to write a run_program()
...
for writing gnump3d::HTML::Template::Expr in the first place.
...
hoped gnump3d::HTML::Template would help them avoid.  To them I'd say that HTML
...
tell us about it.  You can join the gnump3d::HTML::Template mailing-list by
visiting:
...
You can find information about gnump3d::HTML::Template and other related modules at:

   http://html-template.sourceforge.net

=head1 PUBLIC CVS SERVER

gnump3d::HTML::Template now has a publicly accessible CVS server provided by
...
=head1 LICENSE
  ...
  gnump3d::HTML::Template : A module for using HTML Templates with Perl
